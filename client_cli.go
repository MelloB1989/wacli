...
func cmdOpenClaw(args []string) {
	if len(args) == 0 {
		die("usage: wacli openclaw <list|add|update|remove|sessions|deliveries|replay>")
	}
	switch args[0] {
	case "list":
		var response map[string]any
		if err := callLocalAPI(http.MethodGet, "/openclaw_bridges", nil, &response); err != nil {
			die("openclaw list: %v", err)
		}
		prettyPrintJSON(response)
	case "add":
		cmdOpenClawAdd(args[1:])
	case "update":
		cmdOpenClawUpdate(args[1:])
	case "remove", "rm", "delete":
		cmdOpenClawRemove(args[1:])
	case "sessions":
		cmdOpenClawSessions(args[1:])
	case "logs", "deliveries":
		cmdOpenClawDeliveries(args[1:])
	case "replay":
		cmdOpenClawReplay(args[1:])
	default:
		die("usage: wacli openclaw <list|add|update|remove|sessions|deliveries|replay>")
	}
}

func cmdOpenClawAdd(args []string) {
	fs := flag.NewFlagSet("openclaw add", flag.ExitOnError)
	command := fs.String("command", "openclaw", "OpenClaw executable path")
	scope := fs.String("scope", "", "all_unlocked|selected_chats")
	messageTypes := fs.String("message-types", "*", "comma-separated message kinds")
	contextLimit := fs.Int("context-limit", 12, "recent message context window size")
	instruction := fs.String("instruction", "", "instruction/prompt prefix")
	instructionFile := fs.String("instruction-file", "", "read instruction text from file")
	disabled := fs.Bool("disabled", false, "create disabled bridge")
	var chats stringListFlag
	fs.Var(&chats, "chat", "chat reference to subscribe; repeat for multiple chats")
	_ = fs.Parse(args)

	body := buildOpenClawBridgeBody(*command, *scope, splitCSV(*messageTypes), *contextLimit, *instruction, *instructionFile, !*disabled, []string(chats))
	var response map[string]any
	if err := callLocalAPI(http.MethodPost, "/openclaw_bridges", body, &response); err != nil {
		die("openclaw add: %v", err)
	}
	prettyPrintJSON(response)
}

func cmdOpenClawUpdate(args []string) {
	if len(args) == 0 {
		die("usage: wacli openclaw update <id> [flags]")
	}
	id := strings.TrimSpace(args[0])
	fs := flag.NewFlagSet("openclaw update", flag.ExitOnError)
	command := fs.String("command", "", "OpenClaw executable path")
	scope := fs.String("scope", "", "all_unlocked|selected_chats")
	messageTypes := fs.String("message-types", "", "comma-separated message kinds")
	contextLimit := fs.Int("context-limit", 0, "recent message context window size")
	instruction := fs.String("instruction", "", "instruction/prompt prefix")
	instructionFile := fs.String("instruction-file", "", "read instruction text from file")
	...
	body := map[string]any{}
	if flagWasSet(fs, "command") {
		body["command"] = *command
	}
	if flagWasSet(fs, "scope") {
		body["scope"] = *scope
	}
	if flagWasSet(fs, "message-types") {
		body["message_types"] = splitCSV(*messageTypes)
	}
	if flagWasSet(fs, "context-limit") {
		body["context_limit"] = *contextLimit
	}
	if flagWasSet(fs, "instruction") || flagWasSet(fs, "instruction-file") {
		body["instruction"] = mustLoadInstruction(*instruction, *instructionFile)
	}
	if flagWasSet(fs, "enabled") {
		parsed, err := strconv.ParseBool(strings.TrimSpace(*enabled))
		if err != nil {
			die("openclaw update: invalid --enabled value %q", *enabled)
		}
		body["enabled"] = parsed
	}
	if flagWasSet(fs, "chat") {
		body["chat_refs"] = []string(chats)
	}
	if len(body) == 0 {
		die("usage: wacli openclaw update <id> [--command ...] [--scope ...] [--chat ...] [--message-types ...] [--context-limit ...] [--instruction ...|--instruction-file ...] [--enabled true|false]")
	}

	var response map[string]any
	if err := callLocalAPI(http.MethodPut, "/openclaw_bridges/"+url.PathEscape(id), body, &response); err != nil {
		die("openclaw update: %v", err)
	}
	prettyPrintJSON(response)
}

func cmdOpenClawRemove(args []string) {
	if len(args) == 0 {
		die("usage: wacli openclaw remove <id>")
	}
	id := strings.TrimSpace(args[0])
	var response map[string]any
	if err := callLocalAPI(http.MethodDelete, "/openclaw_bridges/"+url.PathEscape(id), nil, &response); err != nil {
		die("openclaw remove: %v", err)
	}
	prettyPrintJSON(response)
}

func cmdOpenClawSessions(args []string) {
	fs := flag.NewFlagSet("openclaw sessions", flag.ExitOnError)
	limit := fs.Int("limit", 200, "maximum session mappings to return")
	queryText := fs.String("query", "", "search by chat name, chat jid, or session id")
	_ = fs.Parse(args)
	...
	var response map[string]any
	if err := callLocalAPIQuery(http.MethodGet, "/openclaw_sessions", query, nil, &response); err != nil {
		die("openclaw sessions: %v", err)
	}
	prettyPrintJSON(response)
}

func cmdOpenClawDeliveries(args []string) {
	fs := flag.NewFlagSet("openclaw deliveries", flag.ExitOnError)
	limit := fs.Int("limit", 100, "maximum delivery logs to return")
	status := fs.String("status", "", "pending|done|failed")
	queryText := fs.String("query", "", "search by chat, session id, message id, output, or error")
	_ = fs.Parse(args)
	...
	var response map[string]any
	if err := callLocalAPIQuery(http.MethodGet, "/openclaw_deliveries", query, nil, &response); err != nil {
		die("openclaw deliveries: %v", err)
	}
	prettyPrintJSON(response)
}

func cmdOpenClawReplay(args []string) {
	fs := flag.NewFlagSet("openclaw replay", flag.ExitOnError)
	bridgeID := fs.Int64("bridge", 0, "openclaw bridge id")
	chatRef := fs.String("chat", "", "chat reference, name, number, chat ID, or JID")
	messageID := fs.String("message-id", "", "specific inbound message id to replay; omit to use the latest inbound message in the chat")
	_ = fs.Parse(args)

	if *bridgeID <= 0 {
		die("usage: wacli openclaw replay --bridge <id> --chat <ref> [--message-id <id>]")
	}
	if strings.TrimSpace(*chatRef) == "" {
		die("usage: wacli openclaw replay --bridge <id> --chat <ref> [--message-id <id>]")
	}
	...
	var response map[string]any
	if err := callLocalAPIWithTimeout(http.MethodPost, "/openclaw_replay", body, &response, 2*time.Minute); err != nil {
		die("openclaw replay: %v", err)
	}
	prettyPrintJSON(response)
}

func buildOpenClawBridgeBody(command, scope string, messageTypes []string, contextLimit int, instruction, instructionFile string, enabled bool, chats []string) map[string]any {
	webhookScope := strings.TrimSpace(scope)
	if webhookScope == "" {
		if len(chats) > 0 {
			webhookScope = "selected_chats"
	...
	return map[string]any{
		...
		"context_limit": contextLimit,
		"instruction":   mustLoadInstruction(instruction, instructionFile),
		"enabled":       enabled,
	}
}

func mustLoadInstruction(instruction, instructionFile string) string {
	if strings.TrimSpace(instructionFile) == "" {
		return instruction
	}
	data, err := os.ReadFile(strings.TrimSpace(instructionFile))
	if err != nil {
		die("read instruction-file: %v", err)
	}
	return string(data)
}
